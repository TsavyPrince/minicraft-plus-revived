...
public class Player extends Mob {
	...
	protected void attack() {
		...
		if (activeItem instanceof ToolItem && stamina - 1 >= 0) {
			...
			if (tool.type == ToolType.Bow && inventory.count(Items.arrowItem) > 0) { // if the player is holding a bow, and has arrows...
				...
				level.addEntity(new Arrow(this, attackDir, tool.level));
	...
	protected void die() {
		...
		if(!Game.ISONLINE)
			World.levels[Game.currentLevel].addEntity(dc);
	...
	protected void doHurt(int damage, Direction attackDir) {
		...
		if(fullPlayer) {
			...
			if(armorDam > 0) {
				level.addEntity(new TextParticle("" + damage, x, y, Color.GRAY));
				armor -= armorDam;
				if(armor <= 0) {
		...
		if(healthDam > 0 || !fullPlayer) {
			level.addEntity(new TextParticle("" + damage, x, y, Color.get(-1, 504)));
