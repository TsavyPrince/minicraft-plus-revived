package minicraft.level.entity.mob;
...
public abstract class Mob extends Entity {
	...
	public void tick() {
		...
		if(xKnockback != 0) {
			moved = moveAxis(xKnockback, 0);
			xKnockback -= xKnockback/Math.abs(xKnockback);
		}
		if(yKnockback != 0) {
			moved = moveAxis(0, yKnockback);
	...
	public boolean move(int xa, int ya) { // Move the mob, overrides from Entity
		...
		if(checkPlayers) {
			...
			for(RemotePlayer rp: prevPlayers)
				Game.server.getAssociatedThread(rp).sendEntityRemoval(this.getEid());
	...
	public void heal(int heal) { // Restore health on the mob
		...
		level.addEntity(new TextParticle("" + heal, x, y, Color.GREEN)); // Add a text particle in our level at our position, that is green and displays the amount healed
