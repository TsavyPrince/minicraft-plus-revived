package minicraft.level.entity;
...
import minicraft.level.entity.mob.MobAi;
...
public abstract class Entity {
	...
	private Level level; // the level that the entity is on
	protected int col; // current color.
	
	private boolean removed = false;
	private int eid; /// this is intended for multiplayer, but I think it could be helpful in single player, too. certainly won't harm anything, I think... as long as finding a valid id doesn't take long...
	...
	public void setEid(int eid) {
		if(this.eid >= 0)
			System.err.println("cannot set entity eid to "+eid+"; already set to " + this.eid);
		else
			this.eid = eid;
	}
	...
	/*public void removeEntity() {
		if(removed && !(this instanceof ItemEntity)) // apparently this happens fairly often with item entities.
			System.out.println("Note: removeEntity() called on removed entity: " + this);
		
		removed = true;
		
		if(level == null)
			System.out.println("Note: removeEntity() called on entity with no level reference: " + getClass());
		else
			level.removeEntity(this);
	}*/
	
	/** This method is solely for with you want to remove an entity and never see it again. */
	public void destroy() {
		removed = true;
		if(level != null)
			level.removeEntity(this);
	...
	public void changeLevel(Level newLevel) {
		if(removed) {
			if (Game.debug) System.out.println("tried to change level of removed entity: " + this);
			return;
		}
		
		if(level == newLevel) {
			if (Game.debug) System.out.println("tried to set level of entity " + this + " to same one it is in: " + level);
			return;
		}
		...
		if(newLevel != null)
			newLevel.addEntity(this);
	}
	
	/** This should ONLY be called by the Level class. To remove an entity, use changeLevel(null). */
	...
	/** This should ONLY be called by the Level class. To properly add an entity to a level, use the changeLevel method. */
	public void setLevel(Level level, int x, int y) {
		if(level == null) {
			throw new NullPointerException("tried to set level of entity " + this + " to a null level; only Level class should call setLevel method, and must pass itself.");
		}
		
		if(removed) {
			if (Game.debug) System.out.println("tried to set level of removed entity: " + this + "; requesting removal from level");
			level.removeEntity(this);
			return;
		}
		
		if(this.level == level) {
			if (Game.debug) System.out.println("tried to set level of " + this + " to level it is already in: " + level);
			return;
		}
	...
	// this was an idea, to hopefully allow the above two methods to be private. I couldn't really make solid sense out of it in the end.
	/*private static class EntityLevel extends Level {
		
		public EntityLevel(int w, int h, int level, Level parentLevel) {
			super(w, h, level, parentLevel);
		}
	...
	public boolean move(int xa, int ya) {
		...
		if(moveAxis(xa, 0)) stopped = false; // becomes false if horizontal movement was successful.
		if(moveAxis(0, ya)) stopped = false; // becomes false if vertical movement was successful.
		if (!stopped) {
	...
	protected boolean moveAxis(int xa, int ya) {
