package minicraft.level.entity.furniture;
...
public class Spawner extends Furniture {
	...
	private void trySpawn() {
		...
		level.addEntity(newmob);
		Sound.monsterHurt.play();
		for(int i = 0; i < 6; i++) {
			randX = rnd.nextInt(16);
			randY = rnd.nextInt(12);
			level.addEntity(new FireParticle(x - 8 + randX, y - 6 + randY));
		}
	}
	...
	public boolean interact(Player player, Item item, Direction attackDir) {
		if(item instanceof ToolItem) {
			...
			health -= dmg;
			level.addEntity(new TextParticle("" + dmg, x, y, Color.get(-1, 200, 300, 400)));
			if(health <= 0) {
