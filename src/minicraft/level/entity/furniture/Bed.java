...
public class Bed extends Furniture {
		...
		if (Updater.tickCount >= Updater.sleepStartTime || Updater.tickCount < Updater.sleepEndTime && Updater.pastDay1) { // if it is late enough in the day to sleep...
			...
			if(Game.isConnectedClient() && player == Game.player) {
				...
				if(Game.isValidServer() && player instanceof RemotePlayer)
					Game.server.getAssociatedThread((RemotePlayer)player).sendEntityRemoval(player.getEid());
			}
		} else {
	...
	public static Player restorePlayer() {
		if(Bed.playerLevel != null) {
			Bed.playerLevel.addEntity(Bed.player); // this adds the player to all the other clients' levels
			if(Game.isValidServer() && player instanceof RemotePlayer)
